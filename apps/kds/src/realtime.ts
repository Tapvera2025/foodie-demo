...
/**
 * Insurance, not polling. See the note above.
 *
 * SHORTER THAN THE CUSTOMER APP'S 60s, and deliberately.
 *
 * PRD §16.1 requires a new ticket to be in front of a kitchen within five
 * seconds. A socket normally delivers that in well under one — but the
 * fallback is what holds when the socket is quietly dead, and on this screen
 * "quietly dead" means a stall silently stops receiving orders it has already
 * been paid for. That is the worst failure in the system, and it is worth
 * paying for cover.
 *
 * A customer's stale screen costs them a refresh; a kitchen's stale screen
 * costs somebody their lunch and the stall its reputation. Different stakes,
 * different insurance premium — one tablet asking every fifteen seconds is
 * nothing, and it is a quarter of the three-second poll it replaces.
 */
