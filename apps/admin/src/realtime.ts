 ...
 *
 * 30s here, between the kitchen's 15s and the customer's 60s.
 *
 * Nobody watching the console is waiting on a specific order the way a cook or
 * a customer is — it is a supervisory view. But it is also the screen someone
 * opens BECAUSE something looks wrong, and a court-wide list is the heaviest
 * query in the system, so neither extreme fits: too fast and one open console
 * tab costs more than the court's customers combined, too slow and the person
 * checking on a problem is looking at stale evidence.
 */
